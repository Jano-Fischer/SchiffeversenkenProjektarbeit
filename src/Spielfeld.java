public abstract class Spielfeld {
    ...
    private GuiSpielfeld gui;

    public void setGui(GuiSpielfeld gui){
        this.gui=gui;
    }
    ...
    public boolean isValid(int x,int y,BoatType boatType,boolean horizontalDirection,boolean lock){
        lock = false;
            for(int j = -1; j < 2; j++) {                                               //zaehler y richtung
                for (int i = -1; i < boatType.getValue() + 1; i++) {     //zaehler x richtung,  for (int i = -1; i < shipsToPlace[arrayPosition].getValue() + 1; i++)
                    if (horizontalDirection) {
